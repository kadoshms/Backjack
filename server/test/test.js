...
describe('Backjack', function(){
	...
	describe('Player', function(){
		it('player ready', function(){
			var sit = {x:1, y:1};
			assert.equal(player.playerReady(sit), true);
			room.playerReady(player.index, sit);

			// After player is ready already...
			assert.equal(player.playerReady(sit), false);
		});
		it('set player credit', function(){
			player.setCredit(500);
			assert.equal(player.getCredit(), 500);
		});
		it('player sit', function(){
			assert.equal(player.sit.x, 1);
			assert.equal(player.sit.y,1);
		})
	});
	...
	describe('Room', function(){
		...
		it('find player by index', function(){
			assert.notEqual(room.getPlayerByIndex(0), null);
		});
